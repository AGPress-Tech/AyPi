...
const { applyTransformPipeline } = require("./transforms");
...
function buildPreviewWithCopyMove(items, transformsConfig, rootPath) {
    ...
    items.forEach((item, index) => {
        ...
        const hasChanges = !error && (newName !== oldName || targetKey !== sourceKey);
        ...
        if (hasChanges) {
            ...
            if (targetUsedByOthers || targetIsOtherOriginal || targetIsExternalExisting) {
                status = "conflict";
                error = "Esiste gia un elemento con lo stesso percorso di destinazione.";
            } else {
                status = "rename";
...
function renderPreviewTable() {
    ...
    state.previewData.forEach((item, index) => {
        ...
        const mtimeLabel =
            item.stats && item.stats.mtime ? new Date(item.stats.mtime).toLocaleString() : "";
