...
type HolidaysPayload = { holidays?: Array<HolidayEntry | string> };

type HolidaysModalOptions = {
    document: Document;
...
function createHolidaysModal(options: HolidaysModalOptions) {
    const {
        document,
        showModal,
        hideModal,
        ...
        requireDaysAccess,
        confirmAction,
    } = options || ({} as HolidaysModalOptions);
    ...
    function normalizeHolidays(payload: HolidaysPayload) {
    ...
    function renderHolidayList(payload: HolidaysPayload, options: { containerId?: string; futureOnly?: boolean } = {}) {
        ...
        const filtered = normalized
            .filter((item) =>
                options.futureOnly ? item.date >= todayKey : true,
            )
        ...
        filtered.forEach(({ date, name }) => {
            ...
            remove.addEventListener("click", () => {
                const run = async () => {
                    const ok = await confirm(
                        "Confermi la rimozione della festivita?",
                    );
                    if (!ok) return;
            ...
            if (editingDate === date) {
                ...
                saveBtn.addEventListener("click", async () => {
                    ...
                    const run = async () => {
                        const ok = await confirm(
                            "Confermi la modifica della festivita?",
                        );
                        if (!ok) return;
                        if (typeof openPasswordModal === "function") {
                            openPasswordModal({
                                ...
                                description:
                                    UI_TEXTS.holidayPasswordDescription,
                                date,
                                nextDate: nextValue,
                                nextName,
                            });
    ...
    function openHolidaysListModal(dateToHighlight?: string) {
        ...
        renderHolidayList(data, {
            containerId: "fp-holidays-future-list",
            futureOnly: true,
        });
        if (highlightDate) {
