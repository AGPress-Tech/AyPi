...
type PendingPanelOptions = {
    document: Document;
    createRangeLine: (document: Document, request: RequestLike) => HTMLElement;
    syncData: (updater: (payload: any) => any) => any;
    renderAll: (data: any) => void;
    getPendingUnlockedBy: () => string;
    getPendingPanelOpen: () => boolean;
    setPendingPanelOpen: (value: boolean) => void;
    updatePendingBadge: (count: number) => void;
    applyBalanceForApproval: (payload: any, request: RequestLike) => void;
    getBalanceImpact: (payload: any, request: RequestLike) => BalanceImpact;
    loadData: () => any;
    confirmNegativeBalance: (impact: BalanceImpact) => Promise<boolean>;
    getLoggedAdminName: () => string;
    onAccessDenied: () => void;
    requireAdminAccess: (run: () => void) => void;
    requireAccess: (adminRequired: boolean, run: () => void) => void;
...
function createPendingPanel(options: PendingPanelOptions) {
    ...
    function renderPendingList(data: { requests?: RequestLike[] }) {
        ...
        pending.forEach((request) => {
            ...
            const deptLabel = request.department
                ? ` - ${request.department}`
                : "";
            ...
            approveBtn.addEventListener("click", async () => {
                const needsAdmin =
                    typeof isAdminRequiredForPendingApprove === "function"
                        ? !!isAdminRequiredForPendingApprove()
                        : true;
                const run = async () => {
                    if (
                        typeof getBalanceImpact === "function" &&
                ...
                if (needsAdmin && typeof requireAccess === "function") {
                    ...
                    needsAdmin &&
                    typeof requireAdminAccess === "function"
                ) {
    ...
    function initPendingPanel() {
        ...
        if (pendingToggle) {
            pendingToggle.addEventListener("click", () => {
                const needsAdmin =
                    typeof isAdminRequiredForPendingAccess === "function"
                        ? !!isAdminRequiredForPendingAccess()
                        : true;
                const run = () => {
