...
function getWrapperId(selectEl: HTMLSelectElement) {
    return `${selectEl.id}-custom`;
}

function getElements(selectEl: HTMLSelectElement) {
...
function closeSelect(wrapper: HTMLElement | null) {
    if (!wrapper) return;
...
function syncCustomSelect(selectEl: HTMLSelectElement) {
...
function ensureCustomSelect(selectEl: HTMLSelectElement) {
    if (!selectEl) return;
    if (!selectEl.id) {
        customSelectId += 1;
    ...
    doc.addEventListener("click", (event) => {
        ...
        if (target && (wrapper.contains(target) || target === selectEl)) return;
        closeSelect(wrapper);
    });
