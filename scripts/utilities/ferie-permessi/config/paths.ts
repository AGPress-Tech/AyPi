...
const DEFAULT_BASE_DIR = (() => {
    try {
        if (NETWORK_PATHS && typeof NETWORK_PATHS.feriePermessiData === "string") {
...
const LEGACY_REQUESTS_PATH = path.join(
    BASE_DIR,
    "ferie-permessi-requests.json",
);
const LEGACY_HOLIDAYS_PATH = path.join(
    BASE_DIR,
    "ferie-permessi-holidays.json",
);
const LEGACY_BALANCES_PATH = path.join(
    BASE_DIR,
    "ferie-permessi-balances.json",
);
const LEGACY_CLOSURES_PATH = path.join(
    BASE_DIR,
