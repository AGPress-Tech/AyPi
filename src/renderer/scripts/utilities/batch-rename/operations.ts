...
async function handleApply(showInfo, showWarning) {
    ...
    if (conflicts > 0) {
        await showWarning(
            "Ci sono conflitti o errori. Risolvili prima di procedere.",
        );
        return;
    }
    ...
    const logCsvPath = path.join(
        logDir,
        `batch-rename-log-${timestampSafe}.csv`,
    );
    const undoScriptPath = path.join(
        logDir,
        `batch-rename-undo-${timestampSafe}.bat`,
    );
    ...
    for (const item of toRename) {
        ...
        const target =
            item.targetFullPath || path.join(targetDir, item.newName);
                ...
                try {
                    ...
                    if (specialCfg.attrHidden && process.platform === "win32") {
                        try {
                            ...
                            console.warn(
                                "Impossibile impostare attributo nascosto per:",
                                op.target,
                                hiddenErr,
                            );
                        }
                    ...
                    console.warn(
                        "Impossibile applicare attributi speciali a:",
                        op.target,
                        attrErr,
                    );
                }
    ...
    try {
        ...
        for (const op of operations.slice().reverse()) {
            if (op.kind === "copy" && !op.isDirectory) {
                ...
                const dst = op.from;
                batLines.push(
                    `if exist "${src}" ren "${src}" "${path.basename(dst)}"`,
                );
            }
        }
    ...
    if (btnUndoLast) {
        btnUndoLast.disabled =
            !state.lastRenameOperations ||
            state.lastRenameOperations.length === 0;
    }
    ...
    setStatus(
        "Rinomina completata. E' stato creato un log e uno script di undo.",
    );
}
...
async function handleOpenFolder(showWarning, showError) {
    ...
    if (!dir) {
        await showError(
            "Impossibile determinare la cartella del file selezionato.",
        );
        return;
    }
