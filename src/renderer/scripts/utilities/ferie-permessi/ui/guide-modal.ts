...
function createGuideModal(options: GuideModalOptions) {
        ...
        try {
            ...
            if (guideSearchParam) {
                ...
                const queryParam = query
                    ? `${encodeURIComponent(guideSearchParam)}=${encodeURIComponent(query)}`
                    : "";
                const base = queryParam
                    ? `${guideUrl}${joiner}${queryParam}`
                    : guideUrl;
    ...
    function tryFindInIframe(frame: HTMLIFrameElement | null, query: string) {
        ...
        try {
            ...
            if (win && typeof (win as any).find === "function") {
                return !!(win as any).find(query);
            }
        } catch (err) {
            return false;
        }
    ...
    function openGuideModal() {
        ...
        if (!guideUrl) {
            if (setMessage)
                setMessage(
                    message,
                    "Guida non configurata. Imposta GUIDE_URL in config/constants.js.",
                );
        } else if (frame) {
            ...
            if (
                ...
                baseUrl &&
                !frame.getAttribute("src")?.includes(baseUrl)
            ) {
                frame.setAttribute("src", baseUrl);
            }
    ...
    return {
        openGuideModal,
        openGuideModalWithQuery,
        openGuideModalAtPath,
        closeGuideModal,
        initGuideModal,
    };
}
