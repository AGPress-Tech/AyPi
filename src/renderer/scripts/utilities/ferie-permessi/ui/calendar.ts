...
type CalendarOptions = {
    ...
    getRequestById?: (id: string | null) => any;
    buildHoverText?: (request: any) => string;
    openPasswordModal?: (payload: any) => void;
    openEditModal?: (request: any) => void;
    getLastNonListViewType?: () => string;
    setLastNonListViewType?: (value: string) => void;
    getHandlingListRedirect?: () => boolean;
    setHandlingListRedirect?: (value: boolean) => void;
};

function applyCalendarButtonStyles(document: Document) {
...
function applyCalendarListStyles(document: Document) {
    const root = document.getElementById("fp-calendar");
    if (!root) return;
    const isDark = document.body.classList.contains("fp-dark");
    const isAyPi = document.body.classList.contains("fp-aypi");
...
function applyCalendarListHoverStyles(document: Document) {
    const root = document.getElementById("fp-calendar");
    if (!root) return;
    const isDark = document.body.classList.contains("fp-dark");
    const isAyPi = document.body.classList.contains("fp-aypi");
