...
const LEGACY_REQUESTS_SHARDS_DIR = path.join(
    path.dirname(LEGACY_REQUESTS_PATH),
    "Calendar Years",
);
...
function getRequestBalanceHours(request: RequestLike, payload: Payload) {
...
function normalizeBalances(payload: Payload, assigneeGroups: Record<string, Array<string | { name?: string }>> | null | undefined, options: { initialHours?: number; monthlyAccrual?: number } = {}) {
    ...
    const nextPayload =
        payload && typeof payload === "object" ? payload : { requests: [] };
    ...
    const currentBalances =
        nextPayload.balances && typeof nextPayload.balances === "object"
    ...
    employees.forEach((row) => {
        ...
        if (!existing) {
            ...
            nextBalances[row.key] = {
                hoursAvailable:
                    Math.round((initialHours - approvedHours) * 100) / 100,
            ...
            if (closureHours > 0) {
                nextBalances[row.key].hoursAvailable =
                    Math.round(
                        (nextBalances[row.key].hoursAvailable - closureHours) *
                            100,
                    ) / 100;
        ...
        if (diff > 0) {
            ...
            const perMonthSafe = Number.isFinite(perMonth)
                ? perMonth
                : monthlyAccrual;
        ...
        if (closureHours !== prevApplied) {
            ...
            existing.hoursAvailable =
                Math.round((Number(existing.hoursAvailable) - delta) * 100) /
                100;
...
function ensureBalanceEntry(payload: Payload, key: string | null) {
    if (!key) return null;
...
function applyBalanceForApproval(payload: Payload, request: RequestLike) {
...
function getBalanceImpact(payload: Payload, request: RequestLike) {
    if (!request || !payload) {
        return {
            negative: false,
            hoursBefore: 0,
...
function applyMissingRequestDeductions(payload: Payload) {
...
function applyBalanceForDeletion(payload: Payload, request: RequestLike) {
    if (!payload || !request) return payload;
    const key = getEmployeeKey(request.employee, request.department);
    if (!key) return payload;
...
function isRequestsManifest(value: unknown) {
    return !!(
        value &&
        typeof value === "object" &&
        !Array.isArray(value) &&
        value.format === "sharded-v1"
    );
}
...
function readRequestsFromShards() {
    ...
    if (primary && legacy) {
        return getLatestShardWriteTimeMs(REQUESTS_SHARDS_DIR) >=
...
function writeRequestsData(requests: RequestLike[] | { requests?: RequestLike[] }) {
    ...
    const legacyShardsEnabled = !!(
        LEGACY_REQUESTS_SHARDS_DIR && fs.existsSync(LEGACY_REQUESTS_SHARDS_DIR)
    );
    ...
    buckets.forEach((items, key) => {
        ...
        if (legacyShardsEnabled) {
            writeJsonFile(
                path.join(LEGACY_REQUESTS_SHARDS_DIR, fileName),
                items,
            );
        }
    });
    ...
    [
        REQUESTS_SHARDS_DIR,
        ...(legacyShardsEnabled ? [LEGACY_REQUESTS_SHARDS_DIR] : []),
    ].forEach((directory) => {
    ...
    try {
        ...
        const requestsFromFiles =
            parsedRequests != null && !isRequestsManifest(parsedRequests)
                ...
                : parsedLegacyRequests != null &&
                    !isRequestsManifest(parsedLegacyRequests)
                  ? normalizeRequestsData(parsedLegacyRequests)
                  : null;

        let requests = null;
        if (requestsFromFiles != null && requestsFromShards != null) {
            const filesMs = Math.max(
                getFileMtimeMs(REQUESTS_PATH),
                getFileMtimeMs(LEGACY_REQUESTS_PATH),
            );
            ...
            requests =
                filesMs >= shardsMs ? requestsFromFiles : requestsFromShards;
        ...
        let holidays =
            parsedHolidays == null
                ? parsedLegacyHolidays == null
                    ? null
                    : normalizeHolidaysData(parsedLegacyHolidays)
                : normalizeHolidaysData(parsedHolidays);
        let balances =
            parsedBalances == null
                ? parsedLegacyBalances == null
                    ? null
                    : normalizeBalancesData(parsedLegacyBalances)
                : normalizeBalancesData(parsedBalances);
        let closures =
            parsedClosures == null
                ? parsedLegacyClosures == null
                    ? null
                    : normalizeClosuresData(parsedLegacyClosures)
        ...
        if (
            needsLegacy &&
            (fs.existsSync(DATA_PATH) || fs.existsSync(LEGACY_DATA_PATH))
        ) {
            const legacyParsed =
                readJsonFile(DATA_PATH) || readJsonFile(LEGACY_DATA_PATH);
            if (requests == null) {
...
function savePayload(payload: Payload) {
    try {
        ...
        if (LEGACY_DATA_PATH && fs.existsSync(LEGACY_DATA_PATH)) {
            writeJsonFile(LEGACY_DATA_PATH, {
                requests,
                balances,
                holidays,
                closures,
